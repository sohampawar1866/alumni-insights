...
export function MembershipBanner({
  ...
  return (
    <>
      {/* Banner Component */}
      <div className="bg-slate-900 text-white rounded-2xl p-6 border-2 border-slate-900 shadow-[6px_6px_0px_#0f172a] relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        {/* Decorative ambient corner */}
        <div className="absolute -top-12 -right-12 w-40 h-40 rounded-full bg-amber-400/10 pointer-events-none" />
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0 z-10 w-full md:w-auto">
          <button
            ...
            className="inline-flex items-center justify-center bg-amber-400 hover:bg-amber-300 text-slate-950 font-bold border-2 border-amber-400 rounded-xl px-5 py-3 text-xs sm:text-sm shadow-[3px_3px_0px_rgba(0,0,0,0.3)] hover:-translate-y-0.5 hover:shadow-[5px_5px_0px_rgba(0,0,0,0.3)] active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 transition-all gap-2"
          >
          ...
          {gformUrl && (
            <a
              ...
              className="inline-flex items-center justify-center bg-white/10 hover:bg-white/20 text-white font-semibold rounded-xl px-4 py-3 text-xs sm:text-sm border border-white/20 active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50 transition-all gap-1.5"
            >
              Google Form Link
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border-2 border-slate-900 rounded-2xl max-w-lg w-full p-6 shadow-[8px_8px_0px_#0f172a] relative space-y-6 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b-2 border-slate-900 pb-4">
              ...
              <button
                ...
                className="min-w-[40px] min-h-[40px] flex items-center justify-center p-1.5 rounded-xl hover:bg-slate-100 text-slate-500 hover:text-slate-900 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 transition-all"
              ...
              <form onSubmit={handleSubmitProof} className="space-y-4 text-xs sm:text-sm">
                ...
                <div className="flex items-center justify-end gap-3 pt-2">
                  <button
                    type="button"
                    onClick={() => setIsOpen(false)}
                    className="px-4 py-2 text-xs font-bold text-slate-700 border-2 border-slate-200 hover:border-slate-900 rounded-xl hover:shadow-[2px_2px_0px_#0f172a] active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 transition-all"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={loading}
                    className="px-5 py-2 text-xs font-bold text-white bg-slate-900 border-2 border-slate-900 rounded-xl shadow-[3px_3px_0px_#0f172a] hover:bg-slate-800 hover:-translate-y-0.5 hover:shadow-[5px_5px_0px_#0f172a] active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100"
                  >
